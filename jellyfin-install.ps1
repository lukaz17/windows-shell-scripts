...
if (!(Test-Path "${INSTALL_TARGET}" -PathType Container)) {
	$BIN_ARCH_URI_AMD64 = "https://repo.jellyfin.org/files/server/windows/stable/v${LATEST_VERSION}/amd64/jellyfin_${LATEST_VERSION}-amd64.zip"
	$BIN_ARCH_URI_ARM64 = "https://repo.jellyfin.org/files/server/windows/stable/v${LATEST_VERSION}/arm64/jellyfin_${LATEST_VERSION}-arm64.zip"
	...
	elseif ("${ARCH}" -eq "ARM64") {
		Invoke-WebRequest -Uri "${BIN_ARCH_URI_ARM64}" -OutFile "${BIN_ARCH_TMP_FILE}"
	}
	else {
	...
	$TEMP_TARGET_FINAL = $([IO.Path]::Combine("${TEMP_TARGET}", "jellyfin"))
