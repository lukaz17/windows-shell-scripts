...
#  qdrant-install
#
#  Install/Update Qdrant Database for current user. This also works as a version manager.
#  If version is not specified, the latest version will be installed.
#
#  Homepage: https://qdrant.tech/qdrant-vector-database/
#
#  Usage:
#    qdrant-install [<version>]
#
#  Example:
#    qdrant-install
#    qdrant-install 1.17.1
#    qdrant-install v1.17.1
#
#  MIT License.
...
if (!(Test-Path "$($InstallEnv.InstallTarget)" -PathType Container)) {
	$BIN_ARCH_URI_AMD64 = "https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases/download/v${INSTALL_VERSION}/qdrant-x86_64-pc-windows-msvc.zip"
	$BIN_ARCH_URI_ARM64 = ""
