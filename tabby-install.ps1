...
#  tabby-install
#
#  Install/Update Tabby for current user. This also works as a version manager.
#  If version is not specified, the latest version will be installed.
#
#  Homepage: https://tabby.sh/
#
#  Usage:
#    tabby-install [<version>]
#
#  Example:
#    tabby-install
#    tabby-install 1.0.227
#    tabby-install v1.0.227
#
#  MIT License.
...
if (!(Test-Path "$($InstallEnv.InstallTarget)" -PathType Container)) {
	$BIN_ARCH_URI_AMD64 = "https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases/download/v${INSTALL_VERSION}/tabby-${INSTALL_VERSION}-portable-x64.zip"
	$BIN_ARCH_URI_ARM64 = "https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases/download/v${INSTALL_VERSION}/tabby-${INSTALL_VERSION}-portable-arm64.zip"
